            ...
            <div class="col-xl-12 col-xxl-12">
                <div class="card">
                    <div class="card-header d-block d-sm-flex border-0">
                        <div class="me-3">
                            <h4 class="card-title mb-2">Histori Transaksi</h4>
                        </div>
                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-responsive-md card-table transactions-table">
                                <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <h6 class="fs-16 font-w600 mb-0">{{ $item->order_id }}</h6>
                                        </td>
                                        <td>
                                            <h6 class="fs-16 font-w600 mb-0">{{ $item->nama }}</h6>
                                        </td>
                                        <td>
                                            <h6 class="fs-16 font-w600 mb-0">{{ $item->event_category }}</h6>
                                        </td>
                                        <td>
                                            <h6 class="fs-16 font-w600 mb-0">{{ $item->ticket_name }}</h6>
                                            @if($item->voucher)
                                            <span class="fs-14">{{$item->voucher}}</span>
                                            @else
                                            <span class="fs-14">-</span>
                                            @endif
                                        </td>
                                        <td>
                                            <h6 class="fs-16 text-black font-w600 mb-0">{{ $item->created_at->format('M d, Y') }}</h6>
                                            <span class="fs-14">{{ $item->created_at->format('h:i:s A') }}</span>
                                        </td>
                                        <td><span class="fs-16 text-black font-w600">Rp. {{ number_format($item->price, 0, ',', '.') }}</span>
                                            <span class="fs-14" style="text-transform: uppercase;">{{$item->pay_method}}</span>
                                        </td>
                                        <td>
                                            @if($item->status_pembayaran === 'paid')
                                            <span class="text-success fs-16 font-w500 text-end d-block">Paid</span>
                                            @elseif($item->status_pembayaran === 'pending')
                                            <span class="text-warning fs-16 font-w500 text-end d-block">Pending</span>
                                            @elseif($item->status_pembayaran === 'failed')
                                            <span class="text-danger fs-16 font-w500 text-end d-block">Failed</span>
                                            @else
                                            <span class="text-muted fs-16 font-w500 text-end d-block">Unknown</span>
                                            @endif
                                        </td>
                                    </tr>
                                    @endforeach
            ...
            fetch('/notifications/data')
                ...
                .then(data => {
                    ...
                    data.forEach(item => {
                        ...
                        const formattedTime = tanggal.toLocaleTimeString('en-US', {
                            hour: '2-digit',
                            minute: '2-digit',
                            second: '2-digit',
                            hour12: true
                        ...
                        const voucher = item.voucher ? item.voucher : '-';
                        let statusBadge = '';
                        if (item.status_pembayaran === 'paid') {
                            statusBadge = `<span class="text-success fs-16 font-w500 text-end d-block">Paid</span>`;
                        } else if (item.status_pembayaran === 'pending') {
                            statusBadge = `<span class="text-warning fs-16 font-w500 text-end d-block">Pending</span>`;
                        } else if (item.status_pembayaran === 'failed') {
                            statusBadge = `<span class="text-danger fs-16 font-w500 text-end d-block">Failed</span>`;
                        } else {
                            statusBadge = `<span class="text-muted fs-16 font-w500 text-end d-block">Unknown</span>`;
                        }

                        tbody += `
        <tr>
            <td><h6 class="fs-16 font-w600 mb-0">${item.order_id}</h6></td>
            <td><h6 class="fs-16 font-w600 mb-0">${item.nama}</h6></td>
            <td><h6 class="fs-16 font-w600 mb-0">${item.event_category}</h6></td>
            <td>
                <h6 class="fs-16 font-w600 mb-0">${item.ticket_name}</h6>
                <span class="fs-14">${voucher}</span>
            </td>
            <td>
                <h6 class="fs-16 text-black font-w600 mb-0">${formattedDate}</h6>
                <span class="fs-14">${formattedTime}</span>
            </td>
            <td>
                <span class="fs-16 text-black font-w600">Rp. ${formattedPrice}</span><br>
                <span class="fs-14 text-uppercase">${item.pay_method}</span>
            </td>
            <td>${statusBadge}</td>
        </tr>`;
                    });

                    document.getElementById('notification-body').innerHTML = tbody;
                });
